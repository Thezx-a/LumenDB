...
## Part 1: Building DeepVector (5 min)
...
cfg.dim = 128
cfg.metric = deepvector.DistanceMetric.L2
cfg.hnsw_m = 16
cfg.hnsw_ef_construction = 200
cfg.hnsw_ef_search = 50
...
# --- Filtered search ---
filter_expr = deepvector.FilterNode.contains("tags", "databases")
...
Expected output:
```
DeepVector initialized, dimension=128
DeepVector server listening on 0.0.0.0:8080
...
# ============================================================
documents = [
    "DeepVector is a C++ vector database optimized for RAG workloads. "
    ...
    "and quantizing each subspace independently. DeepVector supports PQ with configurable "
    ...
    "DeepVector stores vectors using memory-mapped files (mmap) for zero-copy access. "
    ...
    "DeepVector supports metadata filtering through an AST-based filter system. "
...
print(f"Stored {len(coll)} vectors in DeepVector")

# ============================================================
# Step 5: Query the RAG pipeline
# ============================================================
queries = [
    "How does DeepVector store vectors on disk?",
...
```python
# Add this after the DeepVector search in the loop above:
import openai
openai.api_key = "sk-..."
