...
### 2.1 HNSW Index (`include/deepvector/index/hnsw.h`, `src/index/hnsw.cpp`)
...
**File References**:
- `include/deepvector/index/hnsw.h:21-65` — HNSWNode, HNSWIndex class definition
- `src/index/hnsw.cpp:1-175` — insert, search, searchLayer, selectNeighborsSimple, pruneNeighbors
- `include/deepvector/index/distance.h:1-142` — AVX2 and scalar distance kernels

### 2.2 Distance Kernels (`include/deepvector/index/distance.h`)
...
**File References**:
- `include/deepvector/index/distance.h:24-31` — l2_squared_scalar
- `include/deepvector/index/distance.h:35-42` — ip_scalar (negated for minimizing)
- `include/deepvector/index/distance.h:45-53` — cosine_scalar
- `include/deepvector/index/distance.h:56-78` — l2_squared_avx2
- `include/deepvector/index/distance.h:80-95` — ip_avx2
- `include/deepvector/index/distance.h:103-135` — public dispatch functions

### 2.3 Vector Store (`include/deepvector/storage/vector_store.h`, `src/storage/vector_store.cpp`)
...
### 2.4 Document Store / Metadata (`include/deepvector/storage/document_store.h`, `src/storage/document_store.cpp`)
...
**File References**:
- `include/deepvector/storage/document_store.h` — DocumentMeta struct and DocumentStore class
- `src/storage/document_store.cpp` — PIMPL wrapping MiniKV

### 2.5 Quantization (`include/deepvector/quantize/pq.h`, `include/deepvector/quantize/scalar.h`)
...
**File References**:
- `include/deepvector/quantize/pq.h:1-75` — ProductQuantizer API
- `src/quantize/pq.cpp` — k-means training, encode/decode, ADC/SDC, batchADC
- `include/deepvector/quantize/scalar.h:1-28` — ScalarQuantizer API
- `src/quantize/scalar.cpp` — min/max training, encode/decode, l2SquaredDistance

### 2.6 HTTP Server (`include/deepvector/server/server.h`, `src/server/server.cpp`, `src/server/main.cpp`)
...
**File References**:
- `include/deepvector/server/server.h:1-46` — ServerConfig, ServerStats, DeepVectorServer
...
└────────┬─────────┘
         │ HTTP :8080
┌────────▼─────────┐
│  deepvector_server   │  Docker container
│  ┌─────────────┐ │
...
The Dockerfile uses multi-stage builds: g++-12 + CMake in the builder stage, then copies the statically-linked `deepvector_server` binary to a minimal Ubuntu 22.04 runtime image. Final image is <100MB.
...
5. **Non-root user**: The Docker image runs as root; add `USER deepvector` with dropped capabilities.
