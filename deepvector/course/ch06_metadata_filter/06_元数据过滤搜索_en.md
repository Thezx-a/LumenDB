...
For DeepVector, the default is to **NOT index metadata fields** — because the typical use case is batch inserts followed by many reads, and the read path already has a fast filter evaluator. Indexing is opt-in, recommended only when profiling shows a bottleneck.
