...
> **"Code that runs on your laptop ≠ code that runs in production."**

## Prerequisites

This chapter assumes familiarity with the following concepts. Review these shared documents before proceeding:

> 📎 **Reference**: [Docker Containerization](../prerequisites/02_Docker容器化_en.md) — Docker fundamentals, images, containers, Dockerfile basics
> 📎 **Reference**: [Testing Framework](../prerequisites/04_测试框架_en.md) — Google Test and testing fundamentals
> 📎 **Reference**: [Build Environment Configuration](../prerequisites/01_构建环境配置_en.md) — Build tools and compiler setup
...
docker run -d --name deepvector --memory 2g --cpus 2 -p 8080:8080 deepvector:latest

# View logs
docker logs -f deepvector

# Enter container for debugging
docker exec -it deepvector /bin/bash
```
...
> 📎 **Reference**: For Dockerfile fundamentals and multi-stage build concepts, see [Docker Containerization](../prerequisites/02_Docker容器化_en.md).
...
```
COPY CMakeLists.txt /app/     ← Layer 1: cache valid (file unchanged)
COPY cmake/ /app/cmake/       ← Layer 2: cache valid
RUN cmake ...                 ← Layer 3: cache valid (build config unchanged)
COPY src/ /app/src/           ← Layer 4: cache invalidated (source changed)
RUN cmake --build ...         ← Layer 5: must rebuild (upstream invalidated)
```
...
COPY --from=builder /app/build/deepvector /usr/local/bin/
USER deepvector
ENTRYPOINT ["/usr/local/bin/deepvector"]
```
...
| **depends_on** | Declares service startup order. But note: `depends_on` only guarantees container startup order, **not service readiness**—needs health checks to work properly |
...
services:
  deepvector:
    ...
    image: deepvector:latest
    container_name: deepvector
    ports:
      - "8080:8080"                        # host port:container port
    volumes:
      - deepvector_data:/var/lib/deepvector      # Named volume, persistent data
      - ./deepvector.json:/etc/deepvector/config.json:ro  # :ro read-only mount
    environment:
      - DEEPVECTOR_LOG_LEVEL=info
      - DEEPVECTOR_MAX_MEMORY=2GB
    command: ["--config", "/etc/deepvector/config.json"]
  ...
  prometheus:
    ...
    depends_on:
      deepvector:
        condition: service_healthy        # Wait for deepvector to be healthy
...
docker compose logs -f deepvector

# Rolling update deepvector (zero downtime)
docker compose up -d --no-deps --build deepvector

# Scale up (if service is stateless)
docker compose up -d --scale deepvector=3

# Stop all services
docker compose down

# Stop and delete volumes (⚠️ data will be lost!)
...
|------|--------|----------------|
| **Named Volume** | `- deepvector_data:/var/lib/deepvector` | Docker-managed, cross-platform, good performance |
...
| **Pipeline / Workflow** | A series of steps executed sequentially or in parallel: compile → test → static analysis → build image → deploy |
...
Before 2000s:
  ...
  → "It works on my machine" became the classic excuse

2005: CruiseControl (one of the earliest CI tools)

2011: Jenkins (open-source CI server)
  → Required setting up your own server, maintaining plugins
  → Complex configuration, outdated interface

2012: Travis CI (SaaS CI, GitHub integration)
  → Declare pipeline in .travis.yml
...
2019: GitHub Actions
  ...
  → 2000 minutes free per month
  → Now the standard CI/CD solution in the GitHub ecosystem
```
    ...
    steps:
      ...
      - name: Build and push
        ...
        with:
          ...
          push: true
          tags: |
            ${{ secrets.DOCKER_USERNAME }}/deepvector:latest
            ${{ secrets.DOCKER_USERNAME }}/deepvector:${{ github.sha }}
...
1. **Matrix builds**: Multi-platform/multi-compiler parallel testing—building with GCC 12 on Ubuntu 22.04 and GCC 13 on Ubuntu 24.04 ensures cross-platform compatibility
2. **Cache everything**: Build artifacts (build/), compiler cache (ccache), Docker layer caching—every build doesn't start from scratch
...
#      50%    2.1ms        ← P50: 50% of requests complete within this time
#      75%    2.8ms
#      90%    3.5ms        ← P90: 90% of requests complete within this time
#      99%    8.2ms        ← P99: 99% of requests complete within this time
#   1289123 requests in 30.00s, 245MB read
# Requests/sec:  42970.43   ← QPS: requests processed per second
```
...
1. Warmup
   ...
   - CPU L1/L2/L3 caches are finite—first access loads from main memory, which is slow
...
3. Statistical rigor
   ...
   - P99 > P50 × 10 usually indicates severe latency spikes
...
| **SLO (Service Level Objective)** | Internal target, like "P99 latency < 10ms" | Stricter than SLA—leaves margin for yourself |
...
A system without monitoring is like a car without a dashboard—you don't know the speed, fuel level, or engine temperature until an accident happens.
...
| **Metrics** | A timestamped numerical value that changes over time. E.g., `deepvector_search_latency_us{quantile="0.99"} 8200` |
...
public:
    Metrics() {
        search_requests_ = &prometheus::BuildCounter()
            .Name("deepvector_search_requests_total")
            .Help("Total search requests")
            .Register(*registry_);

        search_latency_us_ = &prometheus::BuildHistogram()
            .Name("deepvector_search_latency_us")
...
Prometheus pulls data from `http://deepvector:8081/metrics`, then Grafana connects to the Prometheus data source to draw charts.
...
| Latency (Duration) | P50, P90, P99 | Histogram | P99 > SLA × 2 |
...
Scenario 1: Database is rebuilding index (temporarily cannot handle queries)
  → Readiness Probe fails (doesn't receive new requests)
  → Liveness Probe passes (process is still alive, shouldn't restart)
...
deploy:
  resources:
    limits:
      memory: 2G     # Hard limit, exceeded → OOM kill (kernel forcibly kills process)
      cpus: "2"      # Maximum use of 2 CPU cores' time
    reservations:
      memory: 512M   # Soft reservation: system guarantees at least 512MB

# docker run approach
docker run --memory 2g --memory-swap 2g --cpus 2 deepvector
```
...
```bash
# Create cgroup
sudo mkdir -p /sys/fs/cgroup/deepvector

# Memory limit: 2GB
echo 2147483648 > /sys/fs/cgroup/deepvector/memory.max

# CPU limit: 2 cores (200000 = 2 × 100000)
echo "200000 100000" > /sys/fs/cgroup/deepvector/cpu.max

# Add process to cgroup
echo $PID > /sys/fs/cgroup/deepvector/cgroup.procs
```
...
Example: 1 million vectors × 768 dimensions × 4 bytes = 3GB (vector data)
         HNSW graph (M=16): Each node stores up to M neighbor IDs (8 bytes each) plus
           bidirectional links: total ≈ 2 × M × N × 8 bytes
           = 2 × 16 × 1,000,000 × 8 = 256MB (graph structure)
...
3. P99 latency is much higher than P50 (e.g., P50=1ms, P99=50ms)—what could be the cause? How to locate it? (Hint: think about **tail latency**, **GC pauses**, **lock contention**, **network jitter**)
...
8. In monitoring, QPS suddenly drops to 0 but the process is still running—what's the most likely cause? Write troubleshooting steps. (Hint: check port listening, check load balancer health checks, check DNS)
...
- `deepvector` service (limited to 2GB memory, 2 CPU)
- Persistent volume
...
- Plot QPS vs latency curve (find the inflection point—**knee point**)
...
|--------|-------------|--------------|
| **Docker** | Multi-stage build → image reduced from 500MB-1.5GB to 80MB | Container = namespace + cgroup (not VM); layer caching speeds up builds |
...
> **Remember:** Production is not a "scaled-up" version of development. It requires a different mindset—monitoring, alerting, degradation, rollback, SLA... these are what let you sleep soundly at 3 AM when you get paged.
>
> Next chapter: [Chapter 13: The Capstone — Build Your Own Vector Database](../ch13_capstone/README.md)
