...
> **Preface**: The goal of this chapter is not to make you a C++ expert, but to help you get your development environment running and understand the "why" behind each step. If you've never used a command line before, that's perfectly fine 鈥?we'll start from the most basic concepts. Think of it like learning to drive: first you need to know what the steering wheel, accelerator, and brake are, and then you can hit the road.

## Prerequisites

> 馃搸 **Reference**: [Build Environment Configuration](../prerequisites/01_鏋勫缓鐜閰嶇疆_en.md) 鈥?CMake, compilers, build systems (Ninja vs Make), and compiler flags
...
You may have used **relational databases** like MySQL or PostgreSQL. They organize data into "rows and columns" tables and use SQL for queries. For example, to find "all users older than 25," the database performs an exact match on the `age` column 鈥?25 is exactly 25, not a bit off.
...
- A photo of a cat 鈫?`[0.23, -0.15, 0.87, 0.04, ...]` (possibly 1024 numbers)
- A piece of text "The weather is nice today" 鈫?`[0.12, 0.45, -0.33, 0.78, ...]` (possibly 1536 numbers)
- An audio clip 鈫?`[0.56, -0.22, 0.11, 0.93, ...]` (possibly 2048 numbers)

These number strings are called **embeddings**. They are not randomly generated 鈥?the model, after training on massive datasets, learns to map "semantically similar" things to nearby positions in the numerical space. For example, the vectors for "The weather is nice today" and "The sun is shining brightly today" would be very close, while the vector for "Quantum mechanics is very complex" would be far apart.

### The Problem Vector Databases Solve

Now here's the question: I have 100 million image vectors, a user uploads a new image, and I want to find the "10 most similar" 鈥?how do I do that?

**Brute force**: Compare the new vector against all 100 million vectors one by one. Too slow 鈥?it could take minutes.

**Vector databases** were born to solve this problem. They pre-organize these vectors into a special data structure (called an **index**) to bring search speed from O(n) down to near O(log n). It's like a library's classification system 鈥?you don't have to search the entire library for a book on machine learning; you go straight to the "Computer Science 鈫?Artificial Intelligence 鈫?Machine Learning" shelf.
...
A **terminal** (also called a **command-line interface**, CLI) is a window where you type text commands to control your computer. On Windows, you might have seen PowerShell or CMD; on macOS/Linux, Bash or Zsh are common. Collectively they're called **Shells** 鈥?think of them as a "remote control" for your operating system.
...
4. **Efficiency**: Once proficient, the command line is much faster than a GUI 鈥?like typing is faster than handwriting.

> **Analogy**: A GUI is like an automatic car 鈥?easy to get started, but limited by preset gears. The command line is like a manual transmission 鈥?steeper learning curve, but you have precise control over every detail.
...
Without a compiler, your C++ code is just a pile of useless text 鈥?like writing a cookbook in Chinese, but the chef can only read English. The compiler is that "translator."
...
**CMake** (Cross-Platform Make) is not a compiler, not a build tool 鈥?it's a **build system generator**. This distinction is very important.
...
> **Analogy**: If the compiler is the chef and the build system is the project manager (deciding which chef does what first), then CMake is the "consultant who writes the project management plan" 鈥?it doesn't do the work itself, but the plan it produces determines who does what and in what order.

CMake's core problem to solve: **"One build description, works everywhere."**

Before CMake (before 2000), Linux projects wrote Makefiles, Windows projects wrote Visual Studio `.sln` files, macOS projects wrote Xcode projects 鈥?the same C++ code needed three sets of build files maintained separately. It's like writing a cookbook and having to translate it into Chinese, English, and Japanese versions, changing three places every time you modify a single dish.

CMake generates native build files for each platform from a single `CMakeLists.txt`, completely solving this problem.

> **Historical note**: CMake was developed by Bill Hoffman in 2000 for Kitware, with the original purpose of enabling VTK (Visualization Toolkit, a scientific visualization library) to build cross-platform. Today (2026), CMake is the de facto standard in the C++ world 鈥?even Microsoft Visual Studio natively supports CMake projects.

### CMake 鈫?Ninja Build Flow
...
CMakeLists.txt  鈫? cmake parses  鈫? Makefile / build.ninja  鈫? make / ninja  鈫? Executable
...
|---|---|---|
| **Dependency Resolution** | Must parse variable expansion, function calls, and rule evaluation in the Makefile every build | Build files are static, fully expanded DAGs (Directed Acyclic Graphs) 鈥?zero parsing overhead |
...
> **What is a DAG?** A DAG (Directed Acyclic Graph) is a data structure representing dependencies between tasks. "Directed" means there is direction (A depends on B, not B depends on A), "acyclic" means no circular dependencies (A depends on B, B depends on C, C depends on A 鈥?that would be a problem). Ninja pre-computes all dependencies and doesn't need to analyze them at execution time.

For medium-to-small projects like DeepVector (tens of thousands of lines of code), the gap between Ninja and Make might only be 1-3 seconds. But developing the habit of using Ninja will save you pain when working on large projects. It's like an automatic car 鈥?the difference in daily driving isn't much, but in complex road conditions the experience is vastly different.
...
鈹溾攢鈹€ vendor/MiniKV  鈫?Points to commit abc123 in the MiniKV repository
鈹斺攢鈹€ vendor/SkyNet  鈫?Points to commit def456 in the SkyNet repository
```

### Why `--recurse-submodules`?

A normal `git clone` only pulls the main repository's code 鈥?the submodule directory `vendor/MiniKV/` will be empty! `--recurse-submodules` tells Git: "After cloning the main repository, recursively initialize and pull all submodules."
...
3. **`recursive`**: If submodules contain sub-submodules (nested), pulls those too 鈥?SkyNet has its own internal sub-dependencies

### Project Structure Overview

```
DeepVector/
鈹溾攢鈹€ include/lumendb/     # Public headers (in C++ projects, header files are "interface manuals")
鈹?  鈹溾攢鈹€ collection.h     # Core collection class: responsible for vector CRUD operations
鈹?  鈹溾攢鈹€ config.h         # Global configuration: dimension, distance metric type, etc.
鈹?  鈹溾攢鈹€ distance.h       # Distance calculation interface (supports SIMD acceleration)
鈹?  鈹斺攢鈹€ hnsw.h           # HNSW index: graph edges and layer management
鈹溾攢鈹€ src/                 # Implementation source files (the actual "working" code)
鈹?  鈹溾攢鈹€ collection.cpp   # Collection class implementation
鈹?  鈹溾攢鈹€ distance.cpp     # Distance function scalar/SIMD implementations
鈹?  鈹溾攢鈹€ hnsw.cpp         # Complete HNSW graph construction and search logic
鈹?  鈹斺攢鈹€ storage.cpp      # mmap-based persistent storage layer
鈹溾攢鈹€ tests/               # Test files (the "spotlight" for verifying code correctness)
鈹?  鈹溾攢鈹€ test_basic.cpp
鈹?  鈹溾攢鈹€ test_distance.cpp
鈹?  鈹斺攢鈹€ test_hnsw.cpp
鈹溾攢鈹€ vendor/              # Third-party dependencies (managed via submodules)
鈹?  鈹溾攢鈹€ MiniKV/          # Embedded KV store (lightweight version similar to LevelDB/RocksDB)
鈹?  鈹斺攢鈹€ SkyNet/          # Networking/scheduling library (async I/O and thread pool)
鈹溾攢鈹€ CMakeLists.txt       # Top-level build description file
鈹斺攢鈹€ .github/workflows/   # CI/CD automation configuration
```
...
> **What is HNSW?** HNSW (Hierarchical Navigable Small World) is currently one of the most commonly used vector index algorithms. It organizes vectors into a multi-layer graph structure, and during search it starts from the top layer and progressively refines 鈥?like finding a room in a building: first find the right floor, then the right corridor, then the right room.

> **What is mmap?** mmap (memory-mapped file) is a mechanism provided by the operating system that lets programs access disk files as if they were memory. DeepVector uses it for persistent storage 鈥?vector data is stored on disk, but reads and writes are as fast as memory operations.
...
**`-B build`**: Specifies the build directory as `build/`. This is "out-of-source build" 鈥?the `.o` and `.a` files produced during compilation don't pollute the source directory. This is CMake best practice.

> **What is a `.o` file?** `.o` is an **object file**, an intermediate product produced when the compiler translates a single `.cpp` file. It's not the final executable yet 鈥?all `.o` files need to be compiled first, then the **linker** "stitches" them together into a complete program.

**`-G Ninja`**: Tells CMake to generate Ninja build files (`build.ninja`) instead of the default Makefile. `-G` stands for "Generator" 鈥?you can choose from dozens of backends like `"Unix Makefiles"`, `"Visual Studio 17 2022"`, `"Xcode"`, etc.
...
> **What is `-O3`?** `-O3` is the compiler's "highest optimization level." It tells the compiler: "Make the code run as fast as possible by any means necessary 鈥?a slightly larger binary is fine."
...
- **Auto-vectorization**: Identifies loops that can be vectorized and automatically generates AVX2 instructions 鈥?this is why you get some speedup without hand-writing `_mm256_fmadd_ps`
...
```
build/
鈹溾攢鈹€ libdeepvector.a           # Static library (Release mode)
鈹溾攢鈹€ tests/
鈹?  鈹溾攢鈹€ test_basic         # Test executable (linked with libdeepvector.a)
鈹?  鈹溾攢鈹€ test_distance      # Can run directly, no additional installation needed
鈹?  鈹斺攢鈹€ ...
鈹溾攢鈹€ vendor/
鈹?  鈹斺攢鈹€ MiniKV/libminikv.a # Dependency static library
鈹斺攢鈹€ CMakeFiles/
```

### What is a Library?

A **library** is a packaged collection of pre-compiled `.o` object files containing reusable functions and data for other programs to link against. Libraries exist for "compile once, reuse many times" 鈥?you don't need to recompile `distance.cpp` every time; you just link `libdeepvector.a`.

> **Analogy**: A library is like a LEGO parts pack. You don't need to make each brick from scratch using sand every time 鈥?you just take them out of the pack. Someone else has already made the bricks (compiled them), and you're responsible for assembling (linking).
...
Your program 鈫?Linker extracts needed object files from libdeepvector.a 鈫?Embedded in final executable
```

**How it works**: When the linker processes `-llumendb`, it extracts the object files (`.o`) for functions you actually use from `libdeepvector.a` and **copies** them into the final executable. Functions that aren't called won't be included.

**Advantages**:
- **Self-contained**: The executable you distribute doesn't need `.so` files 鈥?one file runs everything
...
**Disadvantages**:
- **"Tare weight" problem**: If 100 programs use `libdeepvector.a`, 100 copies of DeepVector's code exist on disk 鈥?wasting disk space
- **Update hell**: When a bug is fixed in the library, all downstream programs must be recompiled
- **No shared memory**: The OS can't recognize "these 100 processes are using the same code" 鈥?there are 100 copies in physical memory too
...
Program starts 鈫?Dynamic linker (ld.so) finds libdeepvector.so 鈫?Maps to process address space 鈫?Runs
```
...
- **Memory efficiency**: The OS can store only one copy of the `.so` code segment (text segment) in physical memory, shared by all processes 鈥?this is the true meaning of "shared library"
- **Hot updates**: Replace the `.so` file 鈫?restart the process 鈫?bug fixed, no need to recompile all downstream programs

**Disadvantages**:
- **Dependency hell (DLL Hell)**: A program needs `libdeepvector.so.1`, but the system only has `libdeepvector.so.2` 鈥?the program can't start. Linux's soname mechanism (`libdeepvector.so 鈫?libdeepvector.so.1 鈫?libdeepvector.so.1.0.0` symlink chain) mitigates but doesn't eliminate this problem
- **Symbol conflicts**: Two `.so` files export C++ functions with the same name 鈥?which one gets called? The result depends on load order
- **ABI instability**: C++ doesn't have a stable ABI (Application Binary Interface). An `.so` compiled with g++-12 might not be loadable by a program compiled with g++-13 鈥?because name mangling (the C++ compiler encodes function names into strings containing parameter types, like `_ZN7lumendb8distanceEv`) may have changed
...
2. **Single-process deployment**: Vector search is typically an embedded scenario (embedded in RAG applications), with only one process using DeepVector 鈥?no need for multi-process shared memory.
...
> - `#include <dv/collection.h>`: Includes DeepVector's header file so the compiler knows what the `Collection` class is
...
> - `DistanceMetric::L2`: L2 is Euclidean distance 鈥?the straight-line distance between two points. For two vectors a and b, L2 distance = sqrt((a1-b1)虏 + (a2-b2)虏 + ... + (an-bn)虏)
...
> - `target_link_libraries(...)`: Tells the linker to link against the lumendb and minikv libraries
...
DeepVector uses `add_subdirectory` because MiniKV and SkyNet are in the same repository and developed together with the main project 鈥?they are "source-level dependencies" rather than "binary distribution dependencies." It's like having a `utils/` subdirectory in your own project 鈥?no need to publish it as a standalone library.
...
**CTest** is CMake's built-in test runner, shipped with CMake and requiring no additional installation. The question it answers is: "I have a bunch of test programs 鈥?how do I conveniently run all of them and know which passed and which failed?"

CTest doesn't provide assertions 鈥?it doesn't help you write `EXPECT_EQ(x, 5)`. It only handles:
...
target_link_libraries(test_basic PRIVATE lumendb)
...
> **What is an assertion?** An assertion is a "checkpoint" in a test 鈥?you tell the test framework: "I expect this condition to be true; if it's false, mark the test as failed." For example, `EXPECT_FLOAT_EQ(x, 5.0f)` means "I expect x to equal 5.0; if it doesn't, the test fails."
...
```
GoogleTest (gtest)     鈫?Write test logic: EXPECT_EQ, ASSERT_TRUE, TEST(...)
       鈫?CTest                  鈫?Run and manage: discover, parallelize, summarize, CI integration
```
...
- **CI (Continuous Integration)**: Automatically compile and run tests after every code commit. It solves the problem of "it works on my machine" 鈥?CI ensures it works on others' machines too. Typical flow: `push 鈫?compile 鈫?test 鈫?report results`
- **CD (Continuous Delivery)**: Automatically package releasable artifacts after each CI pass. Typical flow: `CI passes 鈫?compile Release version 鈫?package .tar.gz`
...
> **Analogy**: CI/CD is like a factory assembly line. The traditional way is workers manually moving parts from station A to station B. CI/CD is an automated conveyor belt 鈥?once code is pushed, compilation, testing, and packaging are fully automated, and any step that fails triggers a red flag.

### Why GitHub Actions?

**GitHub Actions** is GitHub's built-in CI/CD service (launched in 2019), running directly in your GitHub repository. Configuration is written in YAML files under the `.github/workflows/` directory. Compared to external services like Jenkins, Travis CI, and CircleCI, GitHub Actions' advantage is zero-configuration integration 鈥?no need to link third-party services to your GitHub account; code is automatically built and tested upon push.
...
The beauty of this **matrix strategy** is: you write the configuration once, and CI automatically generates 4 parallel jobs to verify across two operating systems and two build modes 鈥?ensuring the code compiles on both older Ubuntu 22.04 and latest Ubuntu 24.04; runs correctly in zero-optimization Debug mode (catching uninitialized variables, wild pointers, etc.) and in aggressively optimized Release mode (catching logic errors introduced by optimization, like the compiler "optimizing away" undefined behavior).

Every push triggers this workflow, and GitHub displays a green checkmark (鉁? or red cross (鉁? on the page to tell you whether your code passed all tests.

> **What is a pull_request (PR)?** A PR is Git's collaboration feature. When you've made changes on your own branch and want to merge into the main branch, you can create a PR 鈥?like submitting a "merge application" for others to review your changes, confirming everything is fine before merging.
...
- [ ] CMake's two-phase model: configure (generate) 鈫?build (execute)
...
4. Write a shell script that automatically: pulls code 鈫?configures 鈫?compiles 鈫?runs tests 鈫?summarizes results. Add this script to your GitHub CI workflow.
