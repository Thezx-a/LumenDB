# Chapter 5 鈥?LSM-Tree & MiniKV Internals

## Prerequisites

> 馃搸 **Reference**: [Build Environment Configuration](../prerequisites/01_鏋勫缓鐜閰嶇疆_en.md) 鈥?CMake, compilers, and build commands
> 馃搸 **Reference**: [Testing Framework](../prerequisites/04_娴嬭瘯妗嗘灦_en.md) 鈥?CTest, GoogleTest, assertions, and test organization
...
A **storage engine** is the engine of a database. It's the bottommost layer 鈥?the part that actually puts bytes onto a disk and gets them back. The query parser, the optimizer, the network layer 鈥?all of those are the steering wheel and dashboard. The storage engine is the pistons. It answers two questions: "Where do I put this data?" and "How do I find it again?"

Every database has one. MySQL lets you choose between InnoDB and MyISAM. MongoDB uses WiredTiger. LevelDB and RocksDB *are* storage engines 鈥?libraries that other databases (like CockroachDB, TiKV, and in our case DeepVector's MiniKV) embed.
...
In 1970 (report) / 1972 (journal), Rudolf Bayer and Edward McCreight at Boeing Scientific Research Labs published a paper describing a new data structure: the **B-Tree** (the "B" has never been officially explained 鈥?Bayer says it could stand for "balanced," "broad," or "Boeing").

A **B-Tree** is a balanced tree where each node is exactly one **disk page** (typically 4 KiB, or 4096 bytes). A node with N keys has exactly N+1 children. Because each node is page-sized, reading a node is exactly one disk I/O. Finding any key takes O(log_B N) reads, where B is the **branching factor** (the number of children per node). With B 鈮?500 (a 4 KiB page holding roughly 500 key-pointer pairs), you can find any key among a billion rows in just 3 disk reads.

Think of a B-Tree like a library with a hierarchy of indexes. The root page says "authors A-M are on shelf 2, N-Z on shelf 7." Shelf 2's index says "A-D are in aisle 3, E-M in aisle 5." Aisle 3's index says "Anderson is row 4, Beckett is row 6..." You never scan 鈥?you drill down.
...
The B-Tree was the right answer in the 1970s. Disks were small, RAM was tiny, and databases were measured in megabytes. Reads dominated 鈥?writes were rare. The B-Tree gave you O(log N) reads and writes, which was excellent.

But there was a catch.

### The Write Amplification Problem

Suppose you update a single row in a B-Tree. That row lives on a leaf page. You read the page (one disk seek), modify the row, and write the page back (another seek). But what if the leaf is full? You have to split it. Now the parent node needs updating too. And the grandparent. In the worst case, updating 16 bytes of user data causes the database to write 4096 脳 5 = 20,480 bytes 鈥?over 1000脳 more than the actual change. This ratio 鈥?the total bytes of I/O divided by the bytes of user data 鈥?is called **write amplification**.

**Key term: write amplification.** The ratio of total bytes written to the storage device versus the bytes of actual user data. If you write 1 KB of user data but the storage engine ends up writing 10 KB to disk (due to page rewrites, index updates, journal logging), the write amplification is 10脳. High write amplification wastes I/O bandwidth and shortens SSD lifespan.

Write amplification wasn't a crisis when disks were slow and sequential (HDDs in the 1970s-1990s could do ~100 IOPS 鈥?input/output operations per second). A few extra seeks were fine because there weren't that many writes. But two things changed by the 1990s:
1. Databases grew to gigabytes and terabytes. Indexes got deeper. More levels means more pages to modify per change 鈥?higher write amplification.
...
- **Log-Structured**: The "log" in LSM-Tree refers to the append-only, sequential nature of writes. Data is written to a log (a sequence of entries, like a ship's logbook), never modified in place. This is similar to a **log file** 鈥?a file you only append to, never seek into and overwrite.

Think of a B-Tree as a ledger where you erase old entries and write new ones in the same spot. An LSM-Tree is a ledger where you never erase 鈥?you just keep writing new pages at the bottom. Periodically, you gather up loose pages, sort them, and bind them into a book. This is called **compaction**.

The insight: sequential writes are 100-1000脳 faster than random writes on spinning disks (and even 10脳 faster on SSDs, because flash erase blocks are large). By accepting higher read amplification (you may have to check multiple files to find a key), you get much lower write amplification.
...
graph LR
    ...
    J -->|No| K[Wait]
    J -->|Yes| L[Compaction: Merge L0 鈫?L1]
...
| Read path | Single disk seek, O(log_B N) | Check MemTable 鈫?Bloom 鈫?SSTables |
| Write amplification | 10-50脳 (page rewrites) | 10-30脳 (compaction rewrites, tunable) |
| Read amplification | 1脳 (one read gets you the key) | O(K 脳 log N) where K = number of levels |
| Space amplification | Fragmentation overhead | Temporarily 2脳 during compaction |
...
- **Write-ahead**: A safety protocol: you must write the log entry (the description of the change) *ahead of* (before) you apply the change to the actual data. The word "ahead" is critical 鈥?the log must be on durable storage before any modification is considered committed.
...
- **Durability**: The "D" in ACID (Atomicity, Consistency, Isolation, Durability). Once a transaction is committed, the data must survive any subsequent failure 鈥?power outage, hardware failure, OS crash. The WAL is the mechanism that provides durability.
...
Think of the WAL like a ship's log. Before the captain does anything 鈥?changes course, adjusts speed 鈥?an entry goes into the logbook. If the ship sinks, the logbook (waterproof, floating) tells rescuers what happened. The WAL is the database's waterproof logbook.
...
`fsync` is expensive. On a typical SSD, `fsync` takes about 1 millisecond. If you `fsync` after every write, your maximum throughput is 1000 writes/second 鈥?regardless of how fast your CPU is.

The solution is **group commit**: batch multiple writes together under a single `fsync`. If 1000 writes arrive while you're waiting for the last `fsync` to complete, you write all 1000 of them, call `fsync` once, and they all become durable together. The per-write cost drops from ~1 ms to ~1 碌s. Throughput jumps from 1000 to 1,000,000 writes/second.

The trade-off is latency: a write may wait up to one `fsync` interval (typically 1-10 ms) before being committed. For most applications, this is acceptable 鈥?the 1000脳 throughput gain is worth a few milliseconds of extra latency.
...
- **CRC32** (Cyclic Redundancy Check, 4 bytes): a checksum of the entire frame (including the length field). If the CRC doesn't match, the frame is corrupt. This catches partial writes 鈥?the classic failure mode: the OS writes half the frame, then the power fails. On recovery, the CRC fails and we discard the partial frame.

  **Key term: CRC32 (Cyclic Redundancy Check).** A hash function that takes a block of data and produces a 4-byte checksum. It's used to detect accidental changes to raw data. If even one bit changes in the frame, the CRC will almost certainly be different, allowing the database to detect corruption. CRC32 is not cryptographic 鈥?it's designed for speed, not security.

- **Length** (4 bytes): the total frame length, so we know where the next frame starts.

- **Sequence number** (8 bytes): a monotonically increasing 64-bit counter. Every mutation 鈥?put, delete, transaction begin 鈥?gets a globally unique sequence number. This orders all operations across the entire database.
...
Now imagine adding an "express lane" 鈥?a second linked list that skips every other element. To find 42, you ride the express lane: 7 (still before 42), then 42. You skipped 15 entirely.
...
The brilliance of the SkipList is that the levels are assigned **probabilistically**, not balanced like a tree. When inserting, you flip a coin: heads 鈫?add one more level, tails 鈫?stop. The probability of reaching level k is (1/2)^k. This means:
- Half the nodes are on level 0 (the base list)
- A quarter are on level 1
- An eighth on level 2
- And so on

The result: O(log N) expected time for search, insert, and delete 鈥?with far simpler code than a balanced tree. No rotations, no rebalancing, no color flipping. Just coin flips and pointer splicing.

Pugh's key observation: for a SkipList to work, you don't need perfect balance 鈥?you just need *expected* balance. The coin flips give you that, with high probability.
...
- **Expected time**: The average-case performance over many operations. SkipLists guarantee O(log N) *expected* (average) time, not O(log N) *worst-case* time. In practice, the probability of worse-than-O(log N) is astronomically small (for N = 2^32, the probability of a path longer than 2脳log N is less than 1 in a billion).
...
The disadvantage: SkipLists use ~1.33脳 more memory than balanced trees (the extra pointer arrays), and worst-case behavior is theoretically O(N) 鈥?though astronomically unlikely in practice (a run of 32 consecutive heads before a tails is ~1 in 4.3 billion).

---

## 5.5 The SSTable 鈥?Immutable Sorted File
...
```
鈹屸攢鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹?鈹?Data Block 0 鈹?Data Block 1 鈹?... 鈹?Data Block N-1       鈹?鈹溾攢鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹?鈹?Index Block   (last key of each data block 鈫?offset)     鈹?鈹溾攢鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹?鈹?Bloom Filter  (set of all keys in this SSTable)          鈹?鈹溾攢鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹?鈹?Footer (48 B): index offset, bloom offset, magic number  鈹?鈹斺攢鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹?```
...
## 5.6 Compaction 鈥?Reclaiming Space
...
- **Level 0**: freshly flushed MemTables. Files here may overlap 鈥?two SSTables in level 0 can both contain keys in the range [A, Z].
- **Level 1..N**: each level is roughly 10脳 larger than the previous. Files within a level are sorted and non-overlapping 鈥?a key can exist in at most one file in level 2.
...
The merge is a classic k-way merge using a min-heap: pop the smallest key, write it, advance that file's iterator, push the next key back into the heap. For tombstones (delete markers) at the bottommost level, we can drop them entirely 鈥?they've served their purpose of hiding older values.

---

## 5.7 The Read Path 鈥?Step by Step
...
sequenceDiagram
    ...
    alt Key found in MemTable
        ...
        alt Key found in Immutable
            ...
            ImmutableMemTable-->>L0SSTables: Check L0 SSTables (newest鈫抩ldest)
            loop For each L0 SSTable
                L0SSTables->>L0SSTables: Check Bloom Filter
                alt Bloom says "maybe"
                    L0SSTables->>L0SSTables: Read Index 鈫?Binary Search 鈫?Read Block
...
If a MemTable was recently frozen (it was full and is being flushed), it's still in memory but no longer accepting writes. Check it too 鈥?it's faster than reading from disk.
...
Each SSTable probe may require reading the Bloom filter (a few bytes, or one disk I/O), the index block (one disk I/O), and the data block (one disk I/O). With K levels, the worst case is O(K 脳 3) disk I/Os. In practice, Bloom filters eliminate most probes, so the average case is much better.

**Key term: read amplification.** The number of disk I/O operations required to read a single key. In a B-Tree, read amplification is O(log N) 鈥?the height of the tree. In an LSM-Tree with leveled compaction, it's O(K 脳 log N) where K is the number of levels (each level requires at most one SSTable probe, each probe may require a few I/Os). Read amplification is the price LSM-Trees pay for low write amplification.

---

## 5.8 Compaction Deep Dive 鈥?Why It's Necessary

Compaction isn't optional 鈥?it's essential. Without it, an LSM-Tree would:
...
- **Write amplification**: Compaction rewrites data multiple times. In leveled compaction with a 10脳 growth factor, each byte is rewritten ~10脳 per level. With 7 levels, that's ~70脳 write amplification from compaction alone.
- **Space amplification**: During compaction, both old and new SSTables exist simultaneously, temporarily doubling the space used.
- **I/O bandwidth**: Compaction consumes disk I/O that could otherwise serve user reads and writes. This is why compaction scheduling is critical 鈥?it must not starve foreground operations.

**Key terms:**

- **Space amplification**: The ratio of total storage used by the database versus the actual size of the user data. In an LSM-Tree, space amplification occurs because old SSTables coexist with new ones during compaction. A worst-case space amplification of 2脳 is common during compaction.

- **Read amplification**: (defined above) 鈥?the number of I/O operations needed to read a single key. LSM-Trees have higher read amplification than B-Trees but lower write amplification.

- **Write amplification**: (defined above) 鈥?the ratio of bytes written to disk versus bytes written by the user. LSM-Tree write amplification comes from two sources: (1) writing to the WAL, and (2) rewriting SSTables during compaction.

These three metrics 鈥?read amplification, write amplification, and space amplification 鈥?are called the **three amplification factors** of storage engine design. You can minimize any two at the expense of the third. LSM-Trees optimize for low write amplification at the cost of higher read and space amplification.
...
## 5.9 The Bloom Filter 鈥?Avoiding Unnecessary Reads
...
- **False negative**: A Bloom filter says "no, the key isn't here" when it actually is. Bloom filters never produce false negatives 鈥?this is their defining property.
...
To query: compute all K hash positions. If **any** bit is 0, the element is definitely not in the set. If all bits are 1, the element *might* be in the set 鈥?but it could also be that K different elements happened to set all those bits.

### Why This Matters for SSTables

When searching for a key, the LSM-Tree must potentially check every SSTable in every level. In a database with 1000 SSTables, that's 1000 file reads per query 鈥?even if the key only exists in one of them.

The Bloom filter solves this: before reading an SSTable, check the Bloom filter. If it says "no," skip the SSTable entirely. Reading 64 bytes (the Bloom filter) is ~1000脳 cheaper than reading a 4 KiB block. With a well-configured Bloom filter (1% false positive rate), you eliminate 99% of fruitless SSTable probes.
...
- M 鈮?100,000 脳 9.6 鈮?960,000 bits = 120 KiB
- K 鈮?0.693 脳 9.6 鈮?6.65 鈫?round to 7 hash functions
...
## 5.10 MiniKV 鈥?End-to-End Code Walkthrough

Let's trace a single PUT through the entire MiniKV pipeline:

```
User calls:       db->Put("key_42", "value_42")
                      鈹?WAL::Append()        鈹? 1. Construct frame: CRC | Len | Seq=1001 | Type=PUT | key_42 | value_42
                      鈹? 2. Write frame to WAL file via pwrite()
                      鈹? 3. fsync() for durability
                      鈹?MemTable::Put()       鈹? 4. Insert ("key_42", "value_42", seq=1001) into SkipList
                      鈹?    (O(log N) expected 鈥?flip coins for height, splice pointers)
                      鈹?[MemTable full]       鈹? 5. When MemTable size > 64 MiB:
                      鈹?    a. Freeze current MemTable, create new empty one
SSTableBuilder::Build()鈹?  b. Iterate frozen MemTable in order (walk level 0)
                      鈹?    c. Fill 4 KiB data blocks with prefix-compressed entries
                      鈹?    d. Build index block (last key 鈫?block offset)
                      鈹?    e. Build Bloom filter (add all keys)
                      鈹?    f. Write footer at end of file
                      鈹?    g. Add new SSTable to Level 0 in the manifest
                      鈹?Compaction::          鈹? 6. Monitor level sizes. When Level 0 exceeds threshold:
  MaybeSchedule()     鈹?    a. Pick a file from Level 0
                      鈹?    b. Find all overlapping files in Level 1
                      鈹?    c. K-way merge 鈫?new Level 1 files
                      鈹?    d. Delete old input files, update manifest
```
...
The LSM-Tree is not "better" than a B-Tree 鈥?it's a different trade-off. Here's when to use each:

| Scenario | Best choice | Why |
|---|---|---|
| Read-heavy OLTP (e.g., banking lookups) | B-Tree | Lower read amplification; every key is found in one place |
| Write-heavy workloads (e.g., logs, time-series, vector embeddings) | LSM-Tree | Sequential writes are 100-1000脳 faster than random writes |
| SSD-based storage | LSM-Tree | SSDs have large erase blocks; sequential writes are still much faster |
| Space-constrained | B-Tree | LSM-Tree needs 2脳 space during compaction |
...
4. Implement `Recover`: read frame by frame, check CRC. If a frame is partial (EOF before frame complete) or CRC fails, stop reading 鈥?this is the crash point. Return all valid entries.
5. Test: append 1000 entries, simulate a crash (kill the process mid-loop), recover, verify no lost or corrupted entries.

### Part B 鈥?SkipList MemTable
...
4. **Estimate the write amplification for a 1 TiB database with 64 GiB RAM using leveled compaction with a 10脳 fanout.** How many bytes are written to the storage device per application-level byte over the database's lifetime?

5. **A Bloom filter with M = 1000 bits, K = 3 hashes, N = 100 elements has FP rate 鈮?8.1%. What is the FP rate if K = 7?** Why doesn't increasing K always help?
...
- O'Neil, Patrick, et al. "The log-structured merge-tree (LSM-tree)." *Acta Informatica* 33.4 (1996): 351鈥?85. 鈥?The foundational paper introducing the LSM-Tree concept.
- Chang, Fay, et al. "Bigtable: A distributed storage system for structured data." *OSDI* 2006. 鈥?Introduced the term SSTable and demonstrated LSM-Trees at scale.
- Pugh, William. "Skip lists: a probabilistic alternative to balanced trees." *Communications of the ACM* 33.6 (1990): 668鈥?76. 鈥?Invented the SkipList data structure.
- Bloom, Burton H. "Space/time trade-offs in hash coding with allowable errors." *Communications of the ACM* 13.7 (1970): 422鈥?26. 鈥?Invented the Bloom filter.
- Bayer, Rudolf, and Edward M. McCreight. "Organization and maintenance of large ordered indices." *Acta Informatica* 1.3 (1972): 173鈥?89. 鈥?The original B-Tree paper.
- Facebook RocksDB Wiki, Compaction: https://github.com/facebook/rocksdb/wiki/Compaction 鈥?Practical details on leveled and tiered compaction.
- LevelDB source code: https://github.com/google/leveldb 鈥?Reference implementation of an LSM-Tree storage engine.
