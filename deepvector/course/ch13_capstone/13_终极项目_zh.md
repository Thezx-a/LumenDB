...
### 4.1 与本仓库（DeepVector monorepo）对齐的毕业架构

本仓库已实现的完整栈可直接作为 Capstone 的「参考答案」：

```mermaid
flowchart TB
    subgraph Agent["Agent 层 :8000"]
        MCP[MCP Server]
...
| 里程碑 | 本仓库对应 | 验收标准 |
|--------|-----------|---------|
| Day 1-2 mmap + 暴力搜索 | `vector_store.cpp`, `tests/test_distance.cpp` | 自搜索 recall=100%，重启后数据仍在 |
| Day 3-4 HNSW | `index/hnsw.cpp`, `tests/test_hnsw.cpp` | Recall@10 ≥ 90%（对比暴力） |
| Day 5 过滤 | `filter.cpp`, `tests/test_filter.cpp` | 标签/范围过滤结果正确 |
| Day 6 HTTP + 多 Collection | `server.cpp`, `collection_registry.cpp` | `curl /search` + `/collections` 通过 |
| Day 7 Agent + Docker | `agent/`, `docker-compose.yml`, `RUN.md` | Agent 测试 17/17，`docker compose up` 可演示 |

**7 天增量计划（推荐）：**

1. **Day 1** — mmap 文件头 + append/get；魔数 `DEEPVECTOR`（见 `vector_store.cpp`）
2. **Day 2** — 暴力 top-k + 单元测试
3. **Day 3** — HNSW 插入与单层图
4. **Day 4** — 多层搜索 + recall 基准（`benchmarks/bench_hnsw.cpp`）
5. **Day 5** — 元数据过滤 AST
6. **Day 6** — HTTP REST + Prometheus `/metrics`
7. **Day 7** — FastAPI Agent + MCP + README 设计文档

> 运行与演示命令见 [RUN.md](../../../RUN.md)；OpenAPI 见 [openapi.yaml](../../docs/openapi.yaml)。
...
偏移量  大小    内容
0       8字节   魔数："DEEPVECTOR"（8 字节，见 vector_store.cpp）
8       4字节   向量维度（128）
12      8字节   向量数量
20      8字节   每个向量的字节数（512）
