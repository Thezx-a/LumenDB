# Module 14 â€?Reproduce the Full Project from Scratch

> Goal: chain together everything learned in Modules 01â€?3 and go from `git clone` to an end-to-end working distributed KV platform.
> Prerequisites: Module 01 (environment setup), Module 12 (Go microservices + Next.js console).
> How to read: open two windows â€?this tutorial on the left, a terminal on the right â€?and follow along.
...
In engineering, "reproduce" has a plain definition: **given a clean machine, follow the instructions and end up with a system that runs exactly like the author's**. It is not "reading the code", nor "running a demo" â€?it is the whole process: cloning, installing dependencies, compiling, starting services, and verifying end-to-end, all by yourself.
...
This Module is the third one â€?we're serving TitanKV from scratch.
...
The previous 13 Modules were learned "in blocks" â€?skip list here, LSM there, Raft over there. But a real system is these modules **composed**: browser request â†?Gateway auth â†?Data service â†?storage engine â†?response. Reproducing it once makes you truly understand "what each layer does, why it's designed that way, and how to debug when things break". That's the confidence to talk about the project in interviews, and the scaffold for building your own wheels later.
...
flowchart TD
    ...
    Gateway[Gin Gateway<br/>middleware chain: RequestID â†?Logger â†?Recover<br/>â†?RateLimit â†?Auth â†?RBAC<br/>port 8080] -->|/api/auth/*| Auth
...
1. **Request path**: Browser â†?Next.js (3000) â†?Gateway (8080) â†?four business services. The Gateway is the only externally exposed entry point; business services are never accessed directly by the browser.
...
> Pitfall heads-up: Redis and etcd are both "optional" â€?services degrade gracefully without them (rate limit becomes a no-op, refresh token feature disabled) and won't fail to start. So on your first run you can skip Redis, get the main flow working, then add it.
...
> Why this order? Backend services depend on Go compilation, frontend depends on npm packages â€?the two can be prepared in parallel (stages 3, 4). C++ tests are independent; running them or not doesn't affect the backend. Finally, stage 8 needs both frontend and backend up to do end-to-end.
...
> Pitfall heads-up: Windows users should run backend services (Go + Redis) in WSL2; the frontend can run in PowerShell. macOS users install via Homebrew. If `go version` says "command not found", your PATH isn't set â€?review Module 01's install steps.
...
```
TitanKV/
â”œâ”€â”€ minikv/              # C++17 LSM-Tree storage engine (star of Modules 05-08)
â”œâ”€â”€ skynet/              # C++20 coroutine network library (star of Modules 09-10)
â”œâ”€â”€ gateway/             # Go API gateway (Gin + middleware chain)
â”œâ”€â”€ services/            # Go microservices
â”?  â”œâ”€â”€ auth/            #   Auth service (JWT + APIKey + RBAC)
â”?  â”œâ”€â”€ data/            #   Data service (KV CRUD + SSE Scan)
â”?  â”œâ”€â”€ meta/            #   Meta service (Collection CRUD)
â”?  â””â”€â”€ observability/   #   Observability service (metrics + SSE)
â”œâ”€â”€ client-go/titan/     # Go SDK
â”œâ”€â”€ web/                 # Next.js console
â”œâ”€â”€ tests/course/        # Course unit tests (7 files, standalone build)
â”œâ”€â”€ deploy/dev/          # Docker Compose local dev stack
â”œâ”€â”€ docs/course/         # Bilingual course (the one you're reading)
â”œâ”€â”€ CMakeLists.txt       # Top-level CMake (aggregates minikv + course tests)
â”œâ”€â”€ go.mod               # Go module root
â””â”€â”€ Makefile             # Unified build/test/run entry
```
...
`go mod tidy` will: â‘ fill in missing hashes in `go.sum`; â‘¡remove unused dependencies. **Why it's required**: `go.sum` is the dependency integrity file; CI checks it, and missing/inconsistent entries cause build failures.
...
Expected output: **no output means success** (Go convention â€?successful compilation prints nothing).
...
Each command succeeding is enough. You can `rm` the `/tmp/*` binaries â€?we'll use `go run` later.
...
This stage is optional â€?it doesn't affect the backend services, but it verifies your C++ toolchain and corresponds to the hand-write exercises in Modules 03/05/06.
...
> Pitfall heads-up: the top-level CMake option is `ENABLE_TESTS` (not `ENABLE_COURSE_TESTS`). `tests/course/CMakeLists.txt` is a standalone subproject â€?even if minikv fails to compile, the course tests can run on their own.
...
These tests **don't depend on minikv source** â€?they're pure hand-written implementations, so even without a C++ compiler your backend reproduction is unaffected.
...
sequenceDiagram
    ...
    Note over G: Gateway proxies /api/data â†?:8081<br/>/api/meta â†?:8083<br/>/api/observability â†?:8084
    U->>G: curl localhost:8080/ping
    G-->>U: {"pong":true}
```

> Pitfall heads-up: `make run-all` runs in the foreground and is blocking; `Ctrl+C` stops all services. Logs are interleaved and hard to read â€?when debugging, prefer option B (separate terminals).

### 9.2 Option B: start manually in 5 terminals

Open 5 terminal windows and run each:

**Terminal 1 â€?Auth service (8082)**
...
**Terminal 2 â€?Data service (8081)**
...
**Terminal 3 â€?Meta service (8083)**
...
**Terminal 4 â€?Observability service (8084)**
...
**Terminal 5 â€?Gateway (8080, start last)**
...
> Pitfall heads-up: if you see `[WARN] redis not available`, don't panic â€?this is normal. Without Redis, rate limiting degrades to a no-op and refresh token / jti blacklist features are disabled, but the register/login main flow works fine. To install Redis: `make docker-up` or `docker run -d -p 6379:6379 redis:7`.
...
> Pitfall heads-up: if the page is blank, press F12 and check the Console. Common causes: â‘ `.env.local` is missing `NEXT_PUBLIC_API_BASE`, so requests go to `undefined`; â‘¡the backend isn't running, so requests to 8080 fail; â‘¢CORS errors (see section 13).

---

## 11. Stage 8: End-to-End Verification

This section is the heart of the reproduction â€?we verify all 8 success signs one by one. **Every curl command is copy-paste ready**.
...
Save the `access_token` to an env var â€?all authenticated requests need it:

```bash
TOKEN="paste the access_token value here"
```

> Why two tokens: the access token is short-lived (15 min), the refresh token long-lived. When the access token expires, use the refresh token to get a new one, avoiding repeated password entry. This is the JWT dual-token pattern â€?see Module 12.
...
> Why Collections exist: TitanKV uses a "namespace" model â€?every KV belongs to a Collection, which defines the schema and TTL. The Meta service manages Collection metadata; the Data service manages KV data.
...
> Why the key uses the `user:001` colon form: this is a common KV convention (RocksDB / Redis Hash all do this) â€?the colon acts as a namespace separator, making prefix-range scans convenient.
...
> Why `-N`: `curl -N` disables output buffering; otherwise the SSE stream gets batched and you can't see the "stream" effect. Scan uses SSE because the result set may be large â€?streaming avoids blocking and OOM.
...
2. At the top you'll see live metric cards (QPS / P50 / P99 / Storage) refreshing every second â€?that's SSE at work.
...
1. **The Gateway is the single entry point**: all external requests go through 8080; business services (8081â€?084) are never exposed directly. This is the "BFF / API Gateway" pattern.
...
3. **Reverse proxy routes by path**: `/api/data/*` â†?Data, `/api/meta/*` â†?Meta, `/api/observability/*` â†?Observ. The Gateway uses `httput.ReverseProxy`.
...
**Fix**: `web/app/login/page.tsx` currently sets `SameSite=Lax`, which works for same-site (3000 â†?8080 both on localhost = same site). If you use different domains, change to `SameSite=None; Secure` (requires HTTPS).
...
- No Redis? No problem â€?it degrades with a warning, no error.
...
**Fix**: â‘ configure a proxy; â‘¡or skip the C++ tests (they don't affect backend reproduction); â‘¢or manually download gtest into the build directory.
...
4. Track Phase 4 progress in `docs/REFACTORING.md` â€?once the data service lands, the full reproduction works end-to-end.
...
Every Module maps to real, modifiable code in the project â€?change it, run it, understand why. That's the core of "project-driven learning": **don't just read it once; change a line, run it, and figure out why**.

---

â†?[Module 13 â€?System Design & Interview Q&A](./13-interview.md) | [Back to Course Outline](./README.md) â†?