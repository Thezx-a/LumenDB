...
public:
    ...
    void start() {
        ...
        if (listen_fd_ < 0) {
            std::cerr << "LumenDB: failed to create socket" << std::endl;
            return;
        }
        int opt = 1;
        ...
        if (::bind(listen_fd_, reinterpret_cast<sockaddr*>(&addr), sizeof(addr)) < 0) {
            std::cerr << "LumenDB: failed to bind to " << config_.host << ":" << config_.port << std::endl;
            ::close(listen_fd_);
            listen_fd_ = -1;
            return;
        }
        ...
        running_ = true;
        std::cout << "LumenDB server listening on " << config_.host << ":" << config_.port << std::endl;
...
LumenDBServer::LumenDBServer(const ServerConfig& config, std::unique_ptr<Collection> collection)
    : config_(config), collection_(std::move(collection)) {
    impl_ = std::make_unique<Impl>(config_, collection_.get(), stats_);
}

LumenDBServer::~LumenDBServer() = default;

void LumenDBServer::start() { impl_->start(); }
void LumenDBServer::stop() { impl_->stop(); }
