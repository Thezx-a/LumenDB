...
### 2.1 LumenDB Multi-Stage Dockerfile
...
### 2.2 Dockerfile Layer Caching for LumenDB
...
When your system consists of multiple containers (e.g., LumenDB + Prometheus monitoring + Grafana dashboard), managing each `docker run` command manually is painful.
...
services:
  lumendb:
    ...
    volumes:
      ...
      - ./lumendb.json:/etc/lumendb/config.json:ro  # :ro read-only mount
    environment:
...
For monolithic applications (like early LumenDB), tracing has limited value. But when you split into microservices (like LumenDB frontend + backend + cache layer + vector engine), tracing becomes a powerful tool for locating performance bottlenecks.
...
Write a multi-stage Dockerfile for LumenDB:
