...
本章涵盖 LumenDB 实际使用的模式。每个模式都呈现为：它解决的问题、它的历史、它如何工作、权衡，以及它在 LumenDB 中出现的位置。我们从基础概念开始，逐步构建到并发。
...
本章涵盖 LumenDB 实际使用的模式——不是一个全面的目录，而是那些在真实 C++ 代码库中解决真实问题的模式。
...
LumenDB 中的每个资源都遵循此模式：
- `HNSWIndex` 构造函数分配图的邻接表；析构函数删除它们。
- `VectorStore` 构造函数内存映射数据文件；析构函数 munmap 它。
- `Collection` 析构函数刷新 MemTable 并同步 WAL。

**析构函数永远不应抛出。** C++ 委员会的指导（ISO C++ Core Guidelines C.36）：如果析构函数在栈展开期间抛出（即你已经在处理异常），则调用 `std::terminate`。LumenDB 在析构函数中记录错误但从不传播它们。
...
- **没有内联方法**：实现位于 `.cpp` 文件中，因此编译器不能跨 PIMPL 边界内联。对于热路径方法，LumenDB 暴露返回内部指针的 `getRaw()` 方法，绕过 PIMPL。
...
- **`template <typename F>`**：零成本（单态化）。在类型在编译时已知的热路径中使用。LumenDB 的 SIMD 层使用此方法。

LumenDB 在 `Index` 中使用 `std::function` 作为距离度量：在构造时设置一次，调用数百万次。SBO（在 libstdc++ 上通常 32 字节）适合大多数简单 lambda。
...
LumenDB 的距离函数是单一最热的代码路径——每个查询批次被调用数十亿次。标量（每次操作 1 个浮点数）、SSE（4 个浮点数）和 AVX2（8 个浮点数）之间的性能差异是 4-8 倍。我们想编译一个使用目标 CPU 上最佳可用 SIMD 指令集的二进制文件。
...
没有一种万能的错误处理策略。LumenDB 使用四种机制，每种适合不同的上下文：

| 机制 | 何时使用 | LumenDB 中的示例 |
|-----------|-------------|---------------------|
...
**为什么不用 `std::expected<T, E>`（C++23）？** 它是一个有效的替代方案——比 out 参数更清晰的成功值。但在 LumenDB 设计时，`expected` 不可用。带 out 参数的 `Status` 是务实的选择。

### 工厂模式：Create vs Load

`Collection` 可以来自两个路径——新创建或从磁盘加载。这些有不同的语义和失败模式。LumenDB 将它们分离：
...
这些模式出现在 LumenDB 的后台压缩和 WAL 刷新线程中。它们更高级——在掌握前面的模式后学习它们。
