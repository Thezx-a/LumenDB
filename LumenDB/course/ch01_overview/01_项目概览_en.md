# Chapter 1: Project Overview & Architecture

> LumenDB is a C++ zero-copy vector database. AgenticDB adds an intelligent retrieval layer on top.

## Prerequisites

> 📎 **Reference**: [Build Environment](../prerequisites/01_构建环境配置_en.md) | [Python Environment](../prerequisites/02_Python环境_en.md)

---

## Learning Objectives

- Understand the overall architecture of LumenDB + AgenticDB
- Master the responsibilities and relationships of each module
- Understand the tech stack and design philosophy

---

## 1.1 What is AgenticDB?

```mermaid
flowchart TB
    subgraph User["User Layer"]
        Q["Natural Language Query"]
    end
    subgraph Agent["AgenticDB Agent (Python)"]
        API["HTTP API"]
        LLM["LLM Router<br/>OpenAI/Ollama"]
        EMB["Embedding<br/>Local/OpenAI"]
        ENG["MultiRound Engine<br/>Plan→Search→Evaluate→Answer"]
        MCP["MCP Server"]
    end
    subgraph DB["LumenDB (C++)"]
        HNSW["HNSW Index"]
        MMAP["mmap Store"]
        MINIKV["MiniKV Meta"]
        FILTER["Filter AST"]
        QUANT["PQ/SQ"]
    end
    Q --> API
    API --> ENG
    ENG --> LLM
    ENG --> EMB
    ENG --> DB
    MCP --> DB
```

AgenticDB is not a standalone project. It adds a **Python Agent layer** on top of the LumenDB C++ vector database, enabling:

| Capability | Description | Module |
|-----------|-------------|--------|
| NL Understanding | LLM analyzes question type | `QueryPlanner` |
| Strategy Planning | Choose DIRECT/FILTERED/MULTI_QUERY | `engine/strategy.py` |
| Multi-Round | Iterative result refinement | `MultiRoundEngine` |
| Quality Assessment | LLM judges result sufficiency | `ResultEvaluator` |
| Query Reformulation | Generate improved queries | `QueryReformulator` |
| MCP Integration | Standard protocol for Agent frameworks | `MCP Server` |

---

## 1.2 Project Structure

```
LumenDB/
├── agent/            ← Python Agent Layer (NEW)
│   ├── config.py         Global configuration
│   ├── llm/              LLM integration
│   ├── embedding/        Text embedding service
│   ├── engine/           Retrieval engine
│   ├── server/           HTTP server
│   └── mcp/              MCP protocol
├── src/               ← C++ DB core
│   ├── collection.cpp    Collection API
│   ├── index/hnsw.cpp    HNSW index
│   ├── storage/          mmap storage
│   └── server/           HTTP server
├── tests/             ← Tests
│   └── agent/            17 agent unit tests
├── docs/              ← Documentation
│   ├── AGENTICDB.md      Architecture
│   ├── OPERATIONS.md     Operations manual
│   └── PRODUCTION_QA.md  Interview Q&A
└── course/            ← Course (this directory)
```

---

## 1.3 Tech Stack

| Layer | Technology | Version | Purpose |
|-------|-----------|---------|---------|
| Engine | C++17 + AVX2 | g++-12 | HNSW index + mmap store |
| Network | C++20 coroutines (SkyNet) | - | HTTP server |
| Metadata | MiniKV (LSM-Tree) | - | Document metadata |
| Agent | Python 3.11+ | 3.11 | LLM orchestration |
| LLM Backend | OpenAI / Ollama | - | Model inference |
| Embeddings | sentence-transformers | 5.x | Text vectorization |
| Protocol | MCP (JSON-RPC 2.0) | 1.27 | Agent framework integration |
| Container | Docker + Compose | 24+ | Deployment |

---

## 1.4 Design Philosophy

1. **C++ for compute, Python for AI** — Vector search needs extreme perf, LLM calls are IO-bound
2. **Protocol over SDK** — Integrate via HTTP + MCP, not SDKs
3. **Quality-driven retrieval** — Quality score decides whether to continue, not fixed rounds
4. **Strategy pattern** — 4 retrieval strategies, LLM chooses autonomously
5. **Provider abstraction** — Both LLM and embedding support multiple backends

---

## Review Questions

1. Why use Python for the Agent layer instead of C++? What are the trade-offs?
2. What's the difference between MCP protocol and direct HTTP API? When to use each?
3. What are the pros and cons of "quality-driven retrieval" vs. fixed top-k?

## Hands-on Exercises

1. Run `python -c "from agent.config import load_config; print(load_config())"` to see defaults
2. Read `agent/config.py` to find all overridable environment variables
3. Add a version string to `agent/__init__.py`
