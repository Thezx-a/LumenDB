...
- **Magic number `0x4C4D4442`**: Not random. `0x4C` = 'L', `0x4D` = 'M', `0x44` = 'D', `0x42` = 'B' → "LMDB". If the file doesn't start with this value, it's 100% not a valid LumenDB file — this is the simplest integrity check.
    ...
    void load_existing() {
        ...
        if (header->magic != Header::MAGIC) {
            std::cerr << "Error: Bad magic number! "
                      << "File is not a valid LumenDB data file." << std::endl;
            exit(1);
        }
