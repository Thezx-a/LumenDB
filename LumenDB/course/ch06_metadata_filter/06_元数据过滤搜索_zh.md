...
### 6.2.4 LumenDB 的自适应策略

预过滤和后过滤都不是普遍更好的。LumenDB 在查询时根据估计的选择性做出决定：
...
相反，LumenDB 使用 **FieldAccessor** 模式。对于 schema 中的每个字段，我们存储：
...
这就是自 CSV 文件、JavaScript 和松散类型数据库以来一直困扰的经典"字符串 vs. 数字"混淆。LumenDB 显式处理它：schema 声明每个字段的类型，比较运算符使用适当的比较。

对于可能是任一种的字段（用户上传的数据），LumenDB 使用 `tryNumeric()` 策略：
...
对于 LumenDB，默认是**不索引元数据字段**——因为典型用例是批量插入后跟多次读取，且读取路径已经有快速的过滤求值器。索引是可选的，仅在分析显示瓶颈时推荐。
...
准确的选择性估计是**查询优化**——数据库决定查询最佳执行计划的过程——中最难的问题之一。商业数据库（Oracle、SQL Server）在直方图、草图和抽样上投入了数十年的研究。LumenDB 采用务实的方法。

### 6.7.1 等宽直方图

对于整数字段，LumenDB 维护一个**等宽直方图**：将范围 [min, max] 分为 B 个桶，计算每个桶中有多少文档的值。
