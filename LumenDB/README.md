...
LumenDB is an **embedded** C++ vector database purpose-built for Retrieval-Augmented Generation (RAG). It lives inside your application as a static library — no separate server process, no network overhead, no latency penalty.
...
```mermaid
graph LR
    LumenDB --> MiniKV[MiniKV<br/>LSM-Tree]
    LumenDB --> SkyNet[SkyNet<br/>C++20 Coroutines]
    LumenDB --> pybind11[pybind11<br/>Python Bindings]
    LumenDB --> json[nlohmann/json<br/>JSON Parser]
    LumenDB --> gtest[GoogleTest<br/>Unit Testing]

    style LumenDB fill:#e1f5fe
