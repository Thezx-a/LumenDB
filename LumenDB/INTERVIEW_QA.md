...
| 2 | Layer assignment formula | ⭐⭐ |
| 3 | Search algorithm layer-by-layer | ⭐⭐ |
| 4 | Role of `ef_search` | ⭐ |
| 5 | HNSW time complexity | ⭐⭐ |
| 6 | HNSW deletes (soft-deletion) | ⭐⭐ |
| 7 | Why layer 0 uses `2*M` | ⭐ |
| 8 | `selectNeighborsSimple` vs heuristic | ⭐⭐ |
| 9 | AVX2 L2 squared walkthrough | ⭐⭐⭐ |
| 10 | `_mm256_loadu_ps` vs `_mm256_load_ps` | ⭐⭐ |
| 11 | Why not AVX-512 everywhere? | ⭐⭐ |
| 12 | What is FMA? | ⭐ |
| 13 | Add AVX-512 support | ⭐⭐⭐ |
| 14 | Why not SIMD cosine? | ⭐⭐ |
| 15 | mmap page fault mechanism | ⭐⭐⭐ |
| 16 | `msync(MS_SYNC)` guarantees | ⭐ |
| 17 | `MAP_SHARED` cache coherency | ⭐⭐ |
| 18 | When to use `read/write` vs mmap | ⭐⭐ |
| 19 | `grow` remap safety | ⭐⭐⭐ |
| 20 | LSM-Tree write path | ⭐⭐ |
| 21 | Write amplification & mitigation | ⭐⭐⭐ |
| 22 | Bloom filter in read path | ⭐⭐ |
| 23 | MemTable structure (skip list) | ⭐ |
| 24 | Compaction in MiniKV | ⭐⭐⭐ |
| 25 | PQ high-level explanation | ⭐⭐ |
| 26 | ADC vs SDC | ⭐⭐⭐ |
| 27 | Why K=256 for PQ? | ⭐ |
| 28 | PQ k-means training | ⭐⭐ |
| 29 | `batchADC` efficiency | ⭐⭐ |
| 30 | PQ limitations | ⭐⭐ |
| 31 | PIMPL in LumenDBServer | ⭐⭐ |
| 32 | Distance lambda type erasure | ⭐⭐ |
| 33 | `shared_mutex` vs per-layer locks | ⭐⭐ |
| 34 | `mutable` keyword usage | ⭐ |
| 35 | `const_cast` safety in searchWithFilter | ⭐⭐ |
| 36 | pybind11 NumPy zero-copy | ⭐⭐ |
| 37 | GIL release before C++ calls | ⭐ |
| 38 | `get_vector` zero-copy NumPy view | ⭐⭐ |
| 39 | Buffer protocol (PEP 3118) | ⭐⭐ |
| 40 | LangChain integration structure | ⭐ |
| 41 | `select()` vs epoll | ⭐⭐ |
| 42 | HTTP request parsing | ⭐ |
| 43 | Graceful shutdown | ⭐ |
| 44 | Connection pooling & keep-alive | ⭐⭐ |
| 45 | Rate limiting implementation | ⭐⭐ |
| 46 | Prometheus metrics | ⭐⭐ |
| 47 | Auth beyond bearer tokens | ⭐⭐ |
| 48 | Health check endpoints | ⭐ |
| 49 | LumenDB vs FAISS | ⭐ |
| 50 | LumenDB vs Qdrant | ⭐ |
| 51 | LumenDB vs ChromaDB | ⭐ |
| 52 | LumenDB vs Milvus | ⭐ |
| 53 | When to choose LumenDB | ⭐ |
| 54 | Single-node architecture limits | ⭐⭐ |
| 55 | Scaling to 100M+ vectors | ⭐⭐⭐ |
| 56 | System design: 10K QPS RAG | ⭐⭐⭐ |
| 57 | Testing strategy & HNSW correctness | ⭐⭐ |
| 58 | mmap persistence testing | ⭐⭐ |
| 59 | SIMD vs scalar consistency test | ⭐⭐ |
| 60 | Memory leak detection | ⭐⭐ |
| 61 | Fuzz testing the HTTP parser | ⭐⭐ |
| 62 | Concurrent safety testing | ⭐⭐⭐ |
| 63 | CI pipeline | ⭐ |
| 64 | Regression handling | ⭐ |
| 65 | Server crash mid-write | ⭐⭐⭐ |
| 66 | Debug recall drop from 99% to 80% | ⭐⭐⭐ |
| 67 | Corrupted SSTable handling | ⭐⭐ |
| 68 | Truncated vector file handling | ⭐⭐ |
| 69 | Diagnose OOM on HNSW build | ⭐⭐ |
| 70 | PQ outlier handling | ⭐⭐ |
| 71 | Integrate with OpenAI embeddings | ⭐ |
| 72 | Multi-tenancy | ⭐⭐ |
| 73 | A/B testing index configurations | ⭐⭐ |
| 74 | Monitor vector quality drift | ⭐⭐ |
...
Level `l = floor(-ln(uniform(0,1)) * mL)`, where `mL = 1/ln(M)`. This produces an exponentially decaying distribution — most nodes live only at layer 0, and the probability of reaching level `l` is `(1/M)^l`. This creates the "highway" property: the top layer has few long-range edges enabling logarithmic search complexity. With M=16, ~6.25% of nodes reach level 1, ~0.4% reach level 2.

### Q3: Explain the search algorithm layer by layer.

1. Start at the entry point (topmost layer).
2. For each layer from `max_level_` down to 1: greedily descend with `ef=1` — evaluate distance to all neighbors of the current node, move to the closest, repeat until no improvement. This finds the local minimum at that layer, which becomes the entry point for the next layer down.
3. At layer 0 (the dense layer): run `searchLayer` with `ef_search` candidates. This is a beam search — maintain a candidate priority queue (min-heap by distance) and a result set (max-heap, at most `ef` elements). For each candidate, explore all unvisited neighbors; if a neighbor's distance is better than the worst result, add it to both queues. Terminate when the candidate queue's nearest element is farther than the worst result.
4. Sort the `ef_search` candidates by distance, take top-k.

### Q4: What is the role of `ef_search` and how do you tune it?

`ef_search` controls the search width — the number of candidates explored at layer 0. Higher values increase recall at the cost of latency. The default is `ef_construction=200` set at construction time. For queries:
- `ef_search=16` → 90-95% recall, very fast
- `ef_search=50` → 95-98% recall
- `ef_search=200` → 99%+ recall

Tuning strategy: start with `ef_search = k × 4`, increase until recall meets SLA. The relationship is sublinear — doubling `ef_search` adds ~30% latency for ~2% additional recall.

### Q5: What is the time complexity of HNSW search and insert?

Search: O(log N × M) on average — `log N` layers with `M` neighbor evaluations per step during greedy descent, plus `ef_search × M` evaluations at layer 0. In practice, this is closer to O(log N) for modest `ef_search`.

Insert: O(log N × M × ef_construction) — search for entry points at each of `log N` layers, each using `ef_construction` beam width. The dominant cost is the layer 0 search which explores `ef_construction × M` candidates.
...
Layer 0 is the densest layer (contains all nodes) and carries the bulk of search traffic. Doubling the connections (32 vs 16 for M=16) improves recall at the cost of ~2× memory for neighbor lists. Upper layers are sparse (only ~1/M of nodes per additional layer), so the memory overhead of 2× connections would be negligible but also unnecessary — the greedy descent at upper layers only needs 1 connection to navigate efficiently.

### Q8: What's the difference between `selectNeighborsSimple` and heuristic neighbor selection?

`selectNeighborsSimple` greedily picks the M closest candidates. The heuristic variant (not yet implemented in this codebase) considers whether a candidate is closer to the query than its existing neighbors — it prunes candidates that would create redundant edges. Heuristic selection produces graphs with better navigability and recall but is O(M²) vs O(M log M) for the simple version. The current codebase uses simple selection for speed.
...
Each iteration processes 8 dimensions in ~4 cycles (sub + fmadd can pipeline). For 768-dim, that's 96 iterations at 96×4 ≈ 384 cycles vs scalar's 768×5 ≈ 3840 cycles — roughly 10× speedup on the SIMD portion.
...
4. **Marginal gain for 768-dim vectors**: AVX2 processes 8 floats, AVX-512 processes 16. The 2× theoretical speedup diminishes after amortizing the horizontal sum, tail handling, and the fact that HNSW graph traversal (not distance computation) is often the bottleneck.

### Q12: What is FMA and why does it matter here?

Fused Multiply-Add (`_mm256_fmadd_ps`) computes `a*b + c` with a single rounding step instead of two (multiply then add). For distance computation: `diff*diff + sum` becomes one instruction instead of a multiply followed by an add. Benefits: (1) fewer uops → higher throughput, (2) reduced rounding error since intermediate results aren't rounded, (3) lower latency (4 cycles vs 3+3 for separate mul+add).
...
1. Check if the page is already in the page cache (yes → map PTE, return).
...
4. **Memory-constrained systems**: mmap's page eviction is opaque — the kernel may evict pages you need soon. With `read`, you control buffer lifetime explicitly.
...
1. Write arrives → append to WAL (Write-Ahead Log) → `fsync` if `wal_sync=true`.
2. Insert into MemTable (in-memory sorted structure, typically a skip list).
3. When MemTable reaches size threshold → convert to immutable MemTable, flush to disk as SSTable (Sorted String Table).
4. New MemTable starts accepting writes.

Read path: Check MemTable → check immutable MemTables → check SSTables from newest to oldest (Level 0 may overlap; Level 1+ are non-overlapping). Bloom filters skip SSTables that don't contain the key.

### Q21: What is write amplification and how does MiniKV mitigate it?

Write amplification = (total bytes written to disk) / (bytes of user writes). In LSM-trees, compaction reads multiple SSTables, merges them, and writes new ones — each byte written by the user may be rewritten several times.

Mitigations in MiniKV:
- Leveled compaction: SSTables are organized in levels; Level 0 is ~10MB, Level L+1 is 10× Level L. Data is moved through fewer levels.
...
A Bloom filter is a probabilistic data structure with k hash functions mapping keys to m bits. On put: set bits `h1(key)...hk(key)`. On get: check all k bits — if any is 0, the key is definitely absent (no false negatives). If all are 1, the key might be present (configurable false positive rate).

Read path: `get(key)` → for each SSTable (from newest) → check Bloom filter → if "absent", skip → if "present", do a binary search within the SSTable's index block → read data block → scan for key.
...
This is worst-case write amplification: Level 0 files may overlap with many Level 1 files, potentially rewriting 10× the data. Tiered compaction (RocksDB universal compaction) trades worse read amplification for better write amplification.
...
PQ decomposes the original D-dimensional vector space into M subspaces of dimension `dsub = D/M`. Each subspace has its own codebook of K centroids (trained via k-means). A vector is encoded as M bytes, where byte `i` indexes the closest centroid in subspace `i`'s codebook. The total memory for a vector drops from `D × 4` bytes (float32) to `M` bytes (uint8 codes). For LumenDB default: 768 × 4 = 3072 bytes → 96 bytes (32× compression).

### Q26: What is the difference between ADC and SDC?

**ADC (Asymmetric Distance Computation)**: The query vector is not quantized — it's compared directly against the codebooks. For each subspace, compute the distance from the query's subvector to all K centroids → distance table of size M × K. Then for each candidate code, sum up `dist_table[m][code[m]]`. O(K × dsub × M) for table construction, O(M) per candidate.

**SDC (Symmetric Distance Computation)**: Both query and candidate are quantized. Precompute distance between every pair of centroids within each subspace → M lookup tables of size K × K. Distance between two codes is the sum of `table[m][code_a[m]][code_b[m]]`. O(M × K² × dsub) for precomputation (once), O(M) per candidate.
...
K=256 means each code is 1 byte (uint8_t), which is: (a) the smallest practical integer type, (b) trivially aligned in memory, (c) maps well to CPU byte-addressing, (d) provides enough granularity — beyond 256 centroids per subspace, the marginal recall gain diminishes because the quantization error is dominated by the subspace decomposition, not the codebook resolution.

### Q28: How does LumenDB train PQ using k-means per subspace?

For each subspace `m` in `[0, M)`:
1. Extract subspace `m` from all training vectors → subVectors of size `n × dsub`.
...
4. Store centroids in `centroids_` array: `M × K × dsub` floats.

No k-means++ initialization (random only) — this is a deliberate simplicity trade-off. k-means++ would reduce iterations needed for convergence but adds O(n × K × dsub) time to initialization. For typical training sets (10K+ vectors), random initialization with more iterations converges to comparable quality.

### Q29: How does batchADC work and why is it efficient?

`batchADC(query, codes, n, distances)`:
1. Compute distance table once: O(M × K × dsub).
2. For each of n codes: sum M table lookups: O(n × M).

The distance table construction is amortized over n lookups, making per-vector cost just O(M) table lookups. This is crucial for search — the HNSW search function calls `query_dist_` once per candidate, and if it's the PQ path, each call does the table lookup. For ef_search=50 candidates, that's 50 × 96 = 4800 table lookups vs re-computing full L2 distance (50 × 768 = 38400 operations).
...
The `std::function` erases the lambda type; the HNSW index calls these opaque callbacks without knowing whether PQ, SQ, or raw distance is in use. The cost is one virtual call per distance evaluation — acceptable because the computation inside the call is O(dim).
...
This is safe because: (a) `ef_search_` is `mutable`, so the const_cast is technically unnecessary but expresses intent, (b) the entire operation is within a single function — `ef_search_` is restored before returning, (c) `searchWithFilter` takes exclusive access during the widened search. The alternative (making `setEfSearch` const and marking `ef_search_` mutable) would be cleaner.
...
- `forcecast`: automatically converts compatible dtypes (e.g., float64 → float32).
- `buf.request()`: acquires the buffer protocol → `py::buffer_info` with `.ptr` (raw data pointer) and `.ndim` (dimensions).
...
`Collection::add` performs HNSW graph insertion, which can be computationally heavy (thousands of distance computations). Holding the GIL during this would block all other Python threads, including those not using LumenDB. Releasing the GIL allows true multi-threaded Python programs to run LumenDB operations concurrently with other work. The GIL is re-acquired automatically when `release` goes out of scope.
...
The Python buffer protocol (PEP 3118) is a C-level interface allowing objects to expose their raw memory to other objects without copying. NumPy arrays, `bytes`, `bytearray`, `memoryview`, and `array.array` all implement it. pybind11 uses it so LumenDB bindings work with any buffer-like object, not just NumPy arrays — at the cost of requiring `c_style` and `forcecast` for type safety.

### Q40: How is the LangChain integration structured?

`LumenDBVectorStore` extends LangChain's `VectorStore` base class. It wraps a `Collection` instance and an embedding function. `add_texts` computes embeddings via the provided embedding model, creates `DocumentMeta` objects with text content and optional metadata, and inserts via `add_with_meta`. `similarity_search` embeds the query, calls `search(k)`, retrieves metadata for each result, and returns LangChain `Document` objects. The integration is lazy-imported — if LangChain is not installed, it degrades gracefully.
...
Use a histogram for search latency — `std::chrono::high_resolution_clock` around the search call, record in buckets (100µs, 500µs, 1ms, 5ms, 10ms). The `ServerStats` counters already capture basic counters.
...
- `/health`: Liveness — returns 200 if the process is running. No dependency checks. Used by orchestrator to restart crashed processes.
- `/ready`: Readiness — returns 200 only if the server can accept requests. Checks: mmap is accessible, MiniKV is open, thread pool is alive.
...
### Q49: How does LumenDB compare to FAISS?

| Aspect | LumenDB | FAISS |
|--------|---------|-------|
...
FAISS wins on: index variety, GPU acceleration, and research-backed optimizations (OPQ, polysemous codes). LumenDB wins on: built-in HTTP server, metadata filtering, LangChain integration, and deployment simplicity (single binary).

### Q50: How does LumenDB compare to Qdrant?

| Aspect | LumenDB | Qdrant |
|--------|---------|--------|
| Language | C++ | Rust |
...
Qdrant wins on: distributed operation, rich payload indexing (keyword, integer, geo), gRPC performance, and production maturity. LumenDB wins on: minimal resource footprint, zero-copy mmap performance, C++ embeddability, and codebase simplicity (~3K LOC vs ~100K LOC).

### Q51: How does LumenDB compare to ChromaDB?

| Aspect | LumenDB | ChromaDB |
|--------|---------|----------|
...
ChromaDB wins on: developer experience (built-in embeddings, rich Python API, extensive documentation), production maturity, and community. LumenDB wins on: raw performance (10-50× faster search due to C++ vs Python), memory efficiency, and embeddability in non-Python environments.

### Q52: How does LumenDB compare to Milvus?

| Aspect | LumenDB | Milvus |
|--------|---------|--------|
...
Milvus is a distributed, cloud-native system for billion-scale vector search with tunable consistency and multi-tenancy. LumenDB is an embedded library for single-node RAG workloads. They target completely different scales. LumenDB could serve as Milvus's local index cache (like Milvus Lite), but is not a replacement.

### Q53: When should someone choose LumenDB over alternatives?
...
3. You have **1K-10M vectors** and care about resource efficiency (LumenDB runs comfortably in <100MB RAM with PQ).
...
Don't choose LumenDB when:
1. You need **billion-scale** search → use Milvus or Qdrant cluster.
2. You need **GPU acceleration** → use FAISS.
3. You need **distributed high availability** → use Qdrant with Raft.
4. You need **managed cloud service** → use Pinecone, Zilliz Cloud, or Qdrant Cloud.

### Q54: What's the architecture limitation of a single-node vector database?

1. **Memory ceiling**: If vectors + index don't fit in RAM, disk-based search becomes 100-1000× slower (DiskANN mitigates this with SSD-aware graph layout, but LumenDB doesn't implement it).
...
### Q55: How would you scale LumenDB to 100M+ vectors?
...
5. **Consistent hashing**: Use hash ring for elastic scaling — add/remove nodes without full data reshuffle.

---

## Bonus: System Design Scenario

### Q56: Design a RAG system using LumenDB that handles 10K QPS at P99 < 100ms.

**Architecture**:
- **Ingest pipeline**: Kafka → embedding service (GPU) → LumenDB insert
- **Query pipeline**: API gateway → query embedding → LumenDB search → LLM
- **LumenDB config**: M=64, ef_search=100, PQ enabled with M=96, sharded across 4 nodes by ID range
- **Caching**: Redis cache for frequent queries (embedding → top-k results), TTL=5min
- **Load balancing**: Round-robin across shards for inserts; fan-out to all shards for search, merge with priority queue

**Bottleneck analysis**:
- Embedding generation: ~5ms on GPU → parallelizable
- LumenDB search: ~0.5ms per shard → 0.5ms total (parallel fan-out)
- Network: <1ms intra-DC
- Merge top-k: ~0.1ms with heap
- LLM generation: ~50ms (dominant)
- **Total**: ~56ms → within 100ms budget

**Capacity planning**: 4 shards × 2 replicas = 8 nodes. Each node handles 1.25K QPS for search + 2.5K QPS for insert. HNSW with ef_search=100 at 2.5M vectors per node → well within the <1ms P99 envelope.
...
- **Integration tests**: HNSW — insert N vectors, verify self-search returns self as first result with distance ~0. Collection — insert vectors with metadata, search, verify metadata integrity.
...
2. Call `coll.save()` (which calls `store_->flush()` → `msync(MS_SYNC)`).
...
VectorStore also has a dedicated roundtrip test: create → append → flush → `VectorStore::load()` from the same path → verify count, dim, and data integrity. The `test_collection.cpp` already covers the basic flow.

### Q63: How do you verify SIMD correctness vs scalar?

`test_distance.cpp` includes `SIMDvsScalarConsistency`: generates 100 random vectors of dim=128, computes both SIMD and scalar distances, verifies relative error < 1e-4. This catches off-by-one errors in the SIMD loop, incorrect horizontal sum ordering, and missing tail handling. The test uses `l2_distance` (which returns sqrt of l2_squared internally) and `l2_squared` directly, checking that `l2 * l2 ≈ l2sq`.
...
- **HNSWIndex concurrency design**: `std::shared_mutex` — shared lock for `search()`, exclusive lock for `insert()`. TSAN verifies no unprotected shared state access.
...
All three repos (LumenDB, MiniKV, SkyNet) have independent CI. LumenDB triggers on every push and PR. The pipeline currently runs Release builds (no ASAN/TSAN in CI yet — those are run locally during development). Adding a Debug+Sanitizer matrix job is a planned improvement.
...
- **Correctness regression**: The full test suite runs on every push. Any failing test blocks merging. Tests are designed to be deterministic — fixed RNG seeds (42, 43, 123) ensure reproducible results.
...
**MiniKV WAL**: Guarantees durability. On restart, `DBImpl::recover()` replays WAL records into the MemTable, reconstructing the pre-crash state. SSTables are immutable once written — partial SSTable writes are detected by magic number mismatch and discarded.
...
2. Verify `count ≤ capacity` and file size matches expected (`header_size + capacity * dim * 4`).
3. If mismatch: trust the header count, ignore trailing garbage. The count is the source of truth — it's updated last before `msync`.
...
4. **Check for stale neighbor references**: After many deletes, a node may have neighbors that are all deleted. The search traverses these edges fruitlessly. Run `searchLayer` profiling — count visited nodes per search. Abnormally high visited counts (>ef_search × 10) indicate zombie edges.
5. **Profile with perf**: `perf record -g ./lumendb_server` during search workload. Check for unexpected hotspots — e.g., if `std::function` dispatch overhead is high, the lambda indirection may be the bottleneck.
...
1. **Magic number**: First 4 bytes must match `0x4D494E49` ("MINI"). If not, return `nullptr` — the file is skipped during DB open.
2. **CRC32 per block**: Each data block has a stored CRC32. On read, recompute and compare. If mismatch → skip the block, return partial results. The corrupted block is logged for investigation.
3. **File size check**: The footer (last 48 bytes) contains metadata. If the file is truncated, the footer won't parse — the file is discarded.

Background compaction naturally regenerates corrupted files: the compaction process reads all valid blocks from all files, merges them, and writes fresh SSTables. Corrupted blocks are simply not included in the merge. If corruption is widespread (e.g., disk failure), the DB may lose data — MiniKV doesn't implement Reed-Solomon or replication at the storage layer.
...
2. **Count as source of truth**: On load, `VectorStore::load()` reads the header's `count` field. It verifies that `count ≤ capacity` and that the expected file size (`header_size + capacity * (dim * 4 + 8)`) matches the actual file size (accounting for the ID-to-offset mapping).
3. **Truncation tolerance**: If `count < capacity` (expected after a crash during `append`): trust the header count, ignore trailing garbage. The `nextID` allocator finds the first free slot ≥ 0 and continues from there.
4. **Partial write in data region**: If a vector write was in progress, the partial vector occupies the slot but the `count` field may not have been incremented yet. The partial data is harmless — `nextID` will skip over used-looking slots because the ID-to-offset mapping has that slot's ID (non-zero).

### Q73: How do you diagnose OOM when building HNSW index?

HNSW memory breakdown:
- **Nodes array**: `nodes_.size() × sizeof(HNSWNode)` ≈ `N × (8 + 4 + 24 + 1 + padding)` ≈ `N × 40` bytes. For 1M vectors: ~40MB.
- **Neighbor lists**: Each node at level `l` has up to `M_max0_` (layer 0) or `M_` (upper layers) neighbors, each stored as `uint64_t`. Average neighbor list size: `M_max0_ + Σ(P(level >= l) × M_)` ≈ `2M + (1/M + 1/M² + ...) × M` ≈ `2M + 1`. With M=16: ~33 neighbors × 8 bytes ≈ 264 bytes/node. For 1M: ~264MB.
- **Vector data**: `N × dim × 4` bytes. 768-dim: ~3GB raw.
- **Distance scratch buffers**: `M × K × 4` bytes for PQ distance table. M=96, K=256: ~96KB. Negligible.
- **Total for 1M vectors, M=16, 768-dim raw**: ~3GB + 40MB + 264MB ≈ 3.3GB.
- **With PQ (M=96)**: ~96MB for codes + same index overhead ≈ 400MB.

Monitoring approach:
1. Read `/proc/self/status` → `VmRSS` for RSS, `VmSize` for virtual memory.
2. Track `nodes_.capacity()` × `sizeof(HNSWNode)` for vector overhead.
...
PQ always maps to the nearest centroid regardless of distance — there's no "rejection" mechanism. An outlier vector (far from all centroids) gets encoded to the closest centroid, which may still be very far away. This causes distance estimation error: the PQ-estimated distance is larger than the true distance, leading to false negatives in search.
...
4. **Overlap search**: For critical queries, run a raw L2 distance verification pass on the top 2×k PQ candidates. This catches false negatives caused by distance estimation error at the cost of O(dim × 2k) extra computation.
...
### Q75: How would you integrate LumenDB with OpenAI embeddings?
...
# Ingest documents
documents = ["LumenDB is a vector database", "RAG uses retrieval + generation"]
...
# Search
query_vec = embed("What is LumenDB?")
...
For LangChain users, the built-in `LumenDBVectorStore` wraps this pattern — just pass `OpenAIEmbeddings()` as the embedding function and use `add_texts`/`similarity_search`.

### Q76: How do you handle multi-tenancy?

Strategy 1 — **Separate collections per tenant**:
```cpp
...
Strategy 2 — **Tenant ID in metadata**:
```cpp
meta.tags = "tenant:a";  // or use a dedicated tenant_id field added to DocumentMeta
coll.add_with_meta(vec, meta);
```
Filter every search with `FilterNode::eq("tenant_id", tenant)`. Single collection, simpler deployment, but vectors from different tenants live in the same HNSW graph — the graph traversal visits all vectors, only filtering at the end. This wastes time on cross-tenant distance computations.

Strategy 3 — **JWT-based routing**: The HTTP server extracts `tenant_id` from the JWT claim and routes to the correct Collection instance. Requires a `map<tenant_id, Collection>` on the server side. Auth is handled at the HTTP layer, not in the database.
...
1. **Deploy two LumenDB instances**: Instance A (production) with current config (M=16, ef_search=50, raw). Instance B (experiment) with candidate config (M=32, ef_search=100, PQ).
2. **Dual-write**: All inserts go to both instances.
3. **Shadow-read**: For N% of queries (e.g., 10%), search both instances. Compare results:
   - Recall overlap: `|results_A ∩ results_B| / k`
...
2. **Distance distribution shift**:
   ...
   If P50 shifts from 0.3 to 0.6 → data distribution has changed significantly.
   ```
...
4. **Cluster drift detection**:
   Periodically compute centroid of the last 1000 inserted vectors and compare to global centroid. If cosine distance > threshold (e.g., 0.1), alert — the distribution is changing.
